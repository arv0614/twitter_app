...
<div class="container">
    <div class="row justify-content-center text-center">
        <div class="w-75 mt-5">
            ...
            {{ Form::open(['method'=>'get', 'url'=>'/show_tweets_graph']) }}
            {{ Form::label('search_word', '検索ワード', ['class'=>'search_word']) }}
            {{ Form::text('search_word',  $search_word ) }}
            {{ Form::close() }}
            
            <p><a href="{{ route('show_tweets') }}?search_word={{ $search_word }}">ツイート一覧</a></p>
...
<script>
	...
	<?php
		...
		$max_value2=0;
		foreach($arrays as $index => $array){
			if($max_value1 < $array->cnt){$max_value1=$array->cnt;};
			if($max_value2 < $array->imp){$max_value2=$array->imp;};
		}
	?>
	...
	<?php
		...
		$max_value2=0;
		foreach($arrays as $index => $array){
			if($max_value1 < $array->cnt_rt){$max_value1=$array->cnt_rt;};
			if($max_value2 < $array->cnt_favorite){$max_value2=$array->cnt_favorite;};
		}
	?>
	complexChartOption.scales.yAxes[0].ticks.max= {{ $max_value1 }};
	complexChartOption.scales.yAxes[1].ticks.max= {{ $max_value2 }};
